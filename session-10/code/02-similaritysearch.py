...
# YOU MUST CREATE database (ex. similaritydb) and 'start' it on neo4j gui first before running this script
    ...
    def create_database(self):
        query = """
            CREATE (p:Person {name: 'Ravi Sharma', description: 'AI researcher focusing on healthcare.'}),
                    ...
                    (pr:Project {name: 'AI-Health', description: 'A project exploring AI applications in disease detetion.'})
            WITH, p, f, o, p2, f2, pr
...
def perform_similarity_search(embeddings, sentences, query, model, top_k=3):
    ...
    result = [ (sentences[idx, similarities[0][idx]]) for idx in top_indices ]
    return result
